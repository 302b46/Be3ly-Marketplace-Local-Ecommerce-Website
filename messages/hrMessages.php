...
<body>
<h1 style="text-align:center; color:#033955;">HR messages</h1>
    <form method="GET">
        <input type="text" name="q" placeholder="Type the hr's name">&nbsp;<button type="submit" class="btn">Search availabe HRs</button>
    </form>
    <?php
    if (isset($_GET['u'])) {
    ...
    <?php
         ...
         $users_query = "SELECT * FROM user WHERE UserType=3";
        if (isset($_GET['q'])) {
            $users_query = "SELECT * FROM user WHERE Name LIKE '%" . $_GET['q'] . "%' AND UserType=3" ;
    ...
    ?>
        ...
        <?php
        if ($users->num_rows == 0) {
            echo "<h2>No HRs found</h2>";
        }
		else{
		echo"choose from the HRs:";
		echo"<br>";
		}
